...
contract MySecuredContractWithAdminTransferRecord is AccessControl, ReentrancyGuard {
    ...
    _records[msg.sender][currentNonce] = Record(amount, currentNonce, 0, 0, address(0), block.timestamp);
...
function adminTransfer(address to, uint256 amount) external onlyRole(ADMIN_ROLE) nonReentrant {
    ...
    uint256 currentNonce = _recordNonces[msg.sender];
    _records[msg.sender][currentNonce] = Record(0, 0, amount, currentNonce, to, block.timestamp);
...
function getAdminTransferAmount(address admin, uint256 nonce) public view onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
    return _records[admin][nonce].adminTransferAmount;
}

// 获取管理员的转账Nonce值
function getAdminTransferNonce(address admin) public view onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
    return _recordNonces[admin];
}

// 根据管理员地址和Nonce值获取接收者地址
function getRecipient(address admin, uint256 nonce) public view onlyRole(DEFAULT_ADMIN_ROLE) returns (address) {
